...
#SAVING
    ...
    def load_from_file(self):
        ...
        try:
            ...
            print("Error reading database file. Starting with an empty inventory.")
...
#OPERATIONS
    def add_product(self, pid, name, price, stock):
        ...
        self.log_transaction(f"Added Product: {name} (ID: {pid}, Qty: {stock})")
        self.save_to_file()
    ...
    def update_product(self, pid, name=None, price=None, stock=None):
        ...
        if stock is not None:
            ...
            product.stock = stock
        if changes:
            self.log_transaction(f"Updated Product ID {pid}: " + ", ".join(changes))
            self.save_to_file()
    ...
    def record_sale(self, pid, quantity):
        ...
        self.log_transaction(f"Sale: {product.name} (ID: {pid}, Qty: {quantity}) - Total: ${total:.2f}")
        self.save_to_file()
    ...
    def low_stock_alert(self, threshold=5):
        ...
        for p in self.products.values():
            if p.stock <= threshold:
                alerts.append(f"{p.name} (ID: {p.id}) low stock: {p.stock}")
        return alerts if alerts else ["No low stock items."]


    def calculate_total_value(self):
        total_val = sum(p.price * p.stock for p in self.products.values())
        return total_val
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            print(inv.update_product(pid, name if name else None, price, stock))
            ...
            try:
                thresh_input = input("Enter threshold (leave blank for default of 5): ")
            ...
            query = input("Search by Product ID or Name: ")
            results = inv.search_products(query)
            if not results:
                print("No products found.")
            for p in results:
                print(f"Found -> ID: {p.id}, Name: {p.name}, Price: {p.price}, Stock: {p.stock}")
            ...
            print("Logging out of system. Goodbye!")
            break
